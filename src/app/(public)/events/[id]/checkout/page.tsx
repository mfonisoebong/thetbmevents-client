...
export default function CheckoutPage() {
  ...
  // compute subtotal (ticket sum)
  const subtotal = useMemo(() => ticketInstances.reduce((s, t) => s + (t.price ?? 0), 0), [ticketInstances])

  // coupon discount amount (fixed 1000 NGN if code entered)
  const couponDiscount = couponApplied ? 1000 : 0

  // compute totals depending on selected gateway and coupon
  ...
  useEffect(() => {
    ...
    setPurchaserErrors(prev => {
      try { if (JSON.stringify(prev) === JSON.stringify(pErrs)) return prev } catch (_) {}
      return pErrs
    })
    ...
    setAttendeeErrors(prev => {
      try { if (JSON.stringify(prev) === JSON.stringify(aErrs)) return prev } catch (_) {}
      return aErrs
    })
  ...
  // navigate to step 3
  function goToStep3() {
    // only allow if contact step valid
    if (!isFormValid()) {
      setTouchedAny(true)
      return
    }
    // save customer & attendees before moving to payment
  ...
  async function onPayNow() {
    // final validation
    if (!isFormValid()) { setTouchedAny(true); return }
    if (!gateway) return
    // ensure customer/attendees saved
    setCustomer({ fullname, email, phone })
    setAttendees(localAttendees.map((a: AttendeeLocal) => ({ fullname: a.fullname, email: a.email, phone: a.phone, sendToMe: a.sendToMe })))
          ...
          <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
            ...
            <section className={`bg-white/10 dark:bg-slate-900/40 border border-black/10 dark:border-white/10 rounded-2xl p-6`}>
               <div className="flex items-center gap-3 mb-4">
                 <button type="button" aria-label="Back to contact" onClick={goToStep2} className="text-slate-600 dark:text-slate-300">←</button>
                 <h2 className="text-lg font-semibold dark:text-white">Payment</h2>
               </div>

               <div className="text-sm text-slate-700 dark:text-slate-200 mb-3">Choose a payment method — fees shown to help you pick the cheapest.</div>

               <div className="space-y-3">
                {(['paystack','flutterwave','chainpal'] as PaymentGateway[]).map((g) => {
                  const fee = getGatewayFee(Math.max(0, subtotal - couponDiscount), g)
                  return (
                    <label key={g} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-black/5 dark:bg-black/30 border border-black/10 dark:border-white/10">
                      <div className="flex items-center gap-3 capitalize">
                        <input type="radio" name="gateway" checked={gateway === g} onChange={() => setGateway(g)} />
                        <div className="text-sm text-slate-700 dark:text-slate-200">{g === 'chainpal' ? 'ChainPal (crypto)' : g}</div>
                      </div>
                      <div className="text-sm text-slate-600 dark:text-slate-300">{fee === 0 ? 'Free' : `₦${fee.toLocaleString()}`}</div>
                    </label>
                  )
                })}
               </div>
               ...
               <div className="mt-4">
                ...
                <div className="mt-2 flex gap-2">
                  <input value={coupon} onChange={(e) => { setCoupon(e.target.value); setCouponApplied(false) }} className="w-full rounded-lg bg-white/60 dark:bg-slate-900/50 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-brand-teal" placeholder="Enter coupon code" />
                  <button type="button" onClick={() => { if (coupon.trim()) setCouponApplied(true) }} className="px-4 py-2 rounded-lg bg-brand-teal text-white">Apply</button>
                </div>
                {couponApplied && <div className="mt-2 text-sm text-slate-600 dark:text-slate-300">Coupon applied: ₦1,000 discount</div>}
               </div>
