...
import { computeEventStats, eventStatus, type EventStatus } from '@lib/eventStats'
...
export default function Page() {
  ...
  const derived = useMemo(() => {
    const rows = events.map((e) => {
      ...
      const status = eventStatus(e, stats)
      return { event: e, stats, status }
    })
    ...
    const upcoming = rows
      .filter((r) => new Date(r.event.date).getTime() >= new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime())
    ...
    // "This month" is Jan 2026 in this repo's current date context.
    ...
    () => [
      {
        ...
        sub: <span>Net: {currencySymbol(currency)}{formatNumber(derived.totalNetRevenue)} (-5%)</span>,
        icon: <ArrowTrendingUpIcon className="w-5 h-5" />,
      },
      {
        label: 'Tickets sold',
        value: formatNumber(derived.totalTicketsSold),
        sub: <span>{formatNumber(derived.totalEvents)} event{derived.totalEvents === 1 ? '' : 's'}</span>,
      ...
      },
      {
        ...
        sub: <span>{formatNumber(derived.eventsThisMonth.length)} event{derived.eventsThisMonth.length === 1 ? '' : 's'} in Jan</span>,
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-text-muted-light dark:text-text-muted-dark">
              Quick overview of your events, sales, and check-ins.
            </p>
          </div>
        ...
        <GlassCard className="mt-6 p-6">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-text-muted-light dark:text-text-muted-dark">A tiny feed based on your events (demo).</p>
            </div>
          </div>
