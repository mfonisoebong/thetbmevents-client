...
import { OrganizerFormProps } from "@lib/dashboard-edit-profile/typings"
...
const OrganizerForm: FC<OrganizerFormProps> = ({ user }) => {
  const userData = {
    country: user.country,
    email: user.email,
    dialCode: user.phone_dial_code,
    phoneNumber: user.phone_number,
    buisnessName: user.buisness_name,
  }
  const {
    register,
    formState: { errors },
    watch,
    handleSubmit,
  } = useForm<OrganizationProfileFormType>({
  ...
  const onFormSubmit = (data: OrganizationProfileFormType) => {
