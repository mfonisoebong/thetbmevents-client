...
export default function NavBar(): JSX.Element {
  ...
  const isSignedIn = Boolean(user) || Boolean(role)
  const dashboardHref = role === 'admin' ? '/admin/dashboard' : role === 'organizer' ? '/organizer/dashboard' : '/login'
    ...
    <nav className={`sticky top-0 z-50 backdrop-blur-md transition-all ${showBorder ? 'dark:border-gray-800' : ''}`}>
      <div className="relative z-10 w-full px-6 py-4 flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-6">
          <Link
            className="group flex items-center gap-1 text-sm font-medium text-text-muted-light dark:text-text-muted-dark hover:text-text-light dark:hover:text-white transition-colors"
            href="/explore"
          >
            Explore Events
          ...
          {isSignedIn ? (
            <Link
              ...
              href={dashboardHref}
              title={name ? `Signed in as ${name}` : 'Dashboard'}
              className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-gray-100 dark:bg-gray-800 text-text-light dark:text-text-dark hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors"
            >
